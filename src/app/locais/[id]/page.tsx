...
//   const landmarks: Array<Landmark> = await fetch(
//     "https://380482-3000.csb.app/api/landmark"
//   ).then((res) => res.json());

//   return [{ id: "f96f096e-dacb-4bca-a215-49d8b61bfe41" }];
// }
      ...
      <div className="flex flex-col">
        ...
        <div className="max-w-4xl mx-auto flex lg:flex-row flex-col grid-cols-1 lg:max-w-5xl lg:gap-x-20 lg:grid-cols-2  bg-slate-50 p-10 rounded-md border border-orange-100 transition ease-in-out delay-150 focus:transition-all">
          <div className="grid gap-4 col-start-1 col-end-3 row-start-1 sm:mb-6 sm:grid-cols-4 lg:gap-6 lg:col-start-2 lg:row-end-6 lg:row-span-6 lg:mb-0 lg:w-1/2">
            <Image
              src={landmark.images_url[0]}
              width="500"
              height="500"
              alt=""
              className="transition ease-in-out delay-150 translate-x-0 border-2 border-gray-300 w-full h-60 object-cover rounded-lg sm:h-52 sm:col-span-2 lg:col-span-full"
              loading="lazy"
            />

            {landmark.images_url[1] && (
              <Image
                src={landmark.images_url[1]}
                width="500"
                height="500"
                alt=""
                className="border-2 border-gray-300 hidden w-full h-52 object-cover rounded-lg sm:block sm:col-span-2 md:col-span-1 lg:row-start-2 lg:col-span-2 lg:h-32"
                loading="lazy"
              />
            )}
            ...
            {landmark.images_url[2] && (
              <Image
                src={landmark.images_url[2]}
                width="500"
                height="500"
                alt=""
                className="border-2 border-gray-300 hidden w-full h-52 object-cover rounded-lg md:block lg:row-start-2 lg:col-span-2 lg:h-32"
                loading="lazy"
              />
            )}
          </div>
          ...
          <div className="lg:w-1/2">
            <div className="relative p-3 col-start-1 row-start-1 flex flex-col-reverse rounded-lg bg-gradient-to-t text-bold sm:bg-none sm:row-start-2 sm:p-0 lg:row-start-1">
              <h1 className="mt-1 font-bold text-slate-900 md:text-2xl">
                {landmark.name}
              </h1>
            </div>
